...
dop.def.task_name = 'preschool_testing_manual';
...
dop.data_dir = 'C:\Users\hpayne\Dropbox\Studies\Preschool Doppler\T1\Data\Exps';
%dop.data_dir = 'C:\Users\hpayne\Dropbox\Studies\Junior Doppler\Pilot Junior Doppler\Freezefoot';
%dop.data_dir = '/Users/heathermp/Dropbox/studies/Preschool Doppler/T1/Data/Exps'; %FOR MY MACBOOK
...
if okay
    ...
    for i = 1 : numel(dop.file_list)
        ...
        dop.test.norm = {'overall','epoch'}; %'deppe_epoch'};%{'global','local','baseline'};
        dop.test.base = {'base_correct','no_correct'}; % labels for your file
        
        
        for j = 1 : numel(dop.test.norm)
            % refresh the data to pre-loop
            [dop,okay,msg] = dopUseDataOperations(dop,okay,msg,'hc_linspace');
            
            dop.save.norm = dop.test.norm{j};
            
            [dop,okay,msg] = dopNorm(dop,okay,msg,'norm_method',dop.test.norm{j});
            
            if strcmp(dop.test.norm{j},'overall')
                [dop,okay,msg] = dopEpoch(dop,okay,msg);
                % if not, data has already been epoched
            end
            ...
            for jj = 1 : numel(dop.test.base)
                dop.save.base = dop.test.base{jj};
                [dop,okay,msg] = dopUseDataOperations(dop,okay,msg,'epoch');
                switch dop.save.base
                    case 'base_correct'
                        [dop,okay,msg] = dopBaseCorrect(dop,okay,msg);
                end
